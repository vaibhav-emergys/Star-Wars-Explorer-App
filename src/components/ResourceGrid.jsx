...
// ResourceGrid.jsx (generic grid view)
...
const ResourceGrid = ({ resourceType, searchTerm }) => {
  ...
  const currentItems = filtered.slice(indexOfLastItem - itemsPerPage, indexOfLastItem);
    ...
    <div>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 p-6 justify-items-center">
        {currentItems.map((item) => (
          <ResourceCard key={item.url} item={item} onClick={setSelectedItem} onResourceClick={setSelectedItem} />
        ))}
      </div>
      <Pagination
  ...
  <ResourceModal
    ...
    onResourceClick={(resource) => setSelectedItem(resource)} // ✅ Add this
  />
)}
